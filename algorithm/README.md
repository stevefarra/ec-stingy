...
The signal conditioning filters depend on a set of parameters that depend on the sampling frequency $f_\text{s}$. The paper provides their recommendations for the canonical rate of $f_s = \text{360 H$as well as a procedure for determining some of them given $f_s$.

we first introduce some notation for a moving average filter centered around the current element (assume the signal is zero-padded):
...
acc = acc + new_val / (2*R + 1)
if num_samples > 2*R + 1:
    acc = acc - oldest_val / (2*R + 1)
```
...
For the signal being fed into the reverse template matching filter, $h[i]$, the domain varies from $i-R$ to $i+R$ so the signal buffer size need only be equal to $2R + 1$. The computation is relatively simple so no optimizations are needed.

Our final consideration is the value of $\theta$ in the threshold computation. The paper this algorithm is based upon states that "$\theta$ could be one-fourth of the statistical mean of the output of the low-pass filter", and they go on to use a pre-determined value calculated on their expansive database. Because we don't have this luxury, we leverage the fact that our second low-pass filter uses a large enough window size that the difference between its output and the true mean of the signal is negligible so we can set $\theta = l_2 / 4$ and compute it in real time.

### Latency, time & memory complexity analysis
